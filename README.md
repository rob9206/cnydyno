...
- `components/*.jsx` — source components (Nav, Home, Services, DynoAI, Booking,
  DynoResults, SEO, shared helpers, App).
- `app.js` — precompiled client bundle generated by `_build/prerender.js`,
  loaded with `defer` for hydration (no Babel-in-browser).
...
and `dyno/index.html` from the current component source. Commit the updated
assets, `app.js`, and regenerated HTML.
